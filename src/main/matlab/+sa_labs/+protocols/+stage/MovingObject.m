classdef MovingObject < sa_labs.protocols.StageProtocol
    ...
    properties
        ...
        setSpeeds = 1000                % 
        setDiameters = 2500             % Bar distance (um)
        setShapes = 'circle'            
    ...
    properties (Hidden)
        ...
        currentParameters
        setShapesType = symphonyui.core.PropertyType('char', 'row', {'both', 'left', 'right'})
    ...
    methods
        ...
        function out = parseParamInput(inpt, logScale, twoSided)
            ...
            if twoSided % mostly for offset
                out = horzcat(fliplr(out), out(2:end)); % don't repeat middle value
            end
        end
        ...
        function prepareEpoch(obj, epoch)
            ...
            if index == 0
                obj.directions = obj.angles(randperm(length(obj.directions)));
                ...
                for di = 1:length(obj.directions)
                    for si = 1:length(obj.speeds)
                        for oi = 1:length(obj.offsets)
                            for diami = 1:length(obj.diameters)
                                params = vertcat(params, [obj.directions(ai), obj.speeds(si), obj.offsets(oi), obj.diameters(diami)]);
                            end
                        end
                    end
                end
