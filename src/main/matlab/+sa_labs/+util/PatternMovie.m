classdef PatternMovie < stage.core.Stimulus
% Stimulus that allows drawing channels in a movie one-by-one
% To use:
%   1) set a property controller to alter obj.channel with state.pattern
%   3) set a property controller that increments obj.nextFrame with state.frame

properties
    channel = 1             % Color for the input image (1=red, 2=green, 3=blue)
    nextFrame = 1           % The next frame of the video to draw (can only scrub forward)
...
methods
    ...
    function obj = PatternMovie(filename)
...
methods (Access = protected)
    function performDraw(obj)
        ...
        while obj.frameInd < obj.nextFrame
            ...
            obj.frameInd = obj.frameInd + 1; %the number of frames we've read
        end
        if ~isempty(obj.frame)
            glImage = repmat(obj.frame(:,:,obj.channel), 1, 1, 3);
