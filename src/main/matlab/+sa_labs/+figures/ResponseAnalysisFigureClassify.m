classdef ResponseAnalysisFigureClassify < symphonyui.core.FigureHandler
    ...
    methods
        ...
        function obj = ResponseAnalysisFigureClassify(devices, varargin)
            ...
            ip.addParameter('spikeRateBinLength', 0.01, @(x)isnumeric(x));
...
%                     end
            ...
            %obj.psth <- spike binning
            %want to average multiple runs at same spot size
            %obj.epochData{3}{1} <- contains teh spot size?
            %if epoch num == totalnumepochs
            % obj.ensemble.normalize(obj.psth, binwidth)
            %fprintf to command line
            [m,mi] = ismember(obj.epochData{end}{1}.splitParameter,obj.spotSizes);
                
            if m
                obj.psth(:,mi) = obj.psth(:,mi) + spikeRate_binned';
            else
                obj.spotSizes = cat(1,obj.spotSizes,obj.epochData{end}{1}.splitParameter);
                mi = length(obj.spotSizes);
                obj.psth(:,mi) = spikeRate_binned;
            end
            %obj.spotSizes
            
            obj.redrawPlots();
            
            if numel(obj.epochData) == obj.totalNumEpochs
                global ens;
                global prior;
                
                
                p = struct('SMS_PSTH', obj.psth' ./ (obj.totalNumEpochs./numel(obj.spotSizes)),'cellName','null');
                p.SpotSizeVec = obj.spotSizes';
                p.PSTH_X = (e.t(1)-obj.analysisRegion(1)):obj.spikeRateBinLength:(e.t(end)-obj.analysisRegion(1));
                
                [~,si] = sort(p.SpotSizeVec);
                p.SpotSizeVec = p.SpotSizeVec(si);
                p.SMS_PSTH = p.SMS_PSTH(si,:)./obj.spikeRateBinLength;
                ...
                prob = ens.normalize(p, obj.spikeRateBinLength);%did you load the ens and prior as globals?              
                %disp(prob);
                [~,pi] = max(prob);
                fprintf('\nPredicted class ''%s'' with probability %1.2f (uniform prior)\n',ens.labels{pi},prob(pi));
                %disp(pi);
                fprintf('All classes:\n');
                for n=1:numel(prob)
                    fprintf('\t%s:\t%1.2f\n',ens.labels{n},prob(n));
                ...
                [~,pi] = max(prob);
                %disp(pi);
                fprintf('\nPredicted class ''%s'' with probability %1.2f (given prior)\n',ens.labels{pi},prob(pi));
                fprintf('All classes:\n');
                for n=1:numel(prob)
                    fprintf('\t%s:\t%1.2f\n',ens.labels{n},prob(n));
                end
                
                %note that division by sum is needed to normalize to sum 1
                
                %model output is constructed on a uniform prior
                %thus the output has the form p(class|data) and is
                %proportional to p(class) = 1/numClass
                
                %so to add prior we divide by the uniform prior (multiply
                %by numClass, omitted) and multiply by the new prior
                
                %since the probabilities are only an estimate, the result
                %is not guaranteed to sum to 1, so we renormalize at the
                %end
